...
ARMOR = {  # the worn look, the icons and the helm's 3D model: see armor.py
...
def main():
    ...
    import armor
    armor.write(PACK, os.path.join(ROOT, "tools", "blockbench"))
    write_blood_core()
    write_blood_nova()
...
def write_flat_item(name, models_out, tex_out):
    # The textures (and the helm's 3D model, which replaces this one) come from armor.write.
    with open(os.path.join(models_out, f"{name}.json"), "w") as f:
